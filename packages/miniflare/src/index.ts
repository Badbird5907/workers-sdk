		...
		const socketPath =
			typeof rawEngine === "string"
				...
				: rawEngine === undefined
					? "//./pipe/docker_engine"
					: undefined;
		if (socketPath === undefined || !isNamedPipeAddress(socketPath)) {
			// Already a TCP address, or a full engine object -- workerd can consume it directly.
			return rawEngine;
		}
		this.#dockerProxy ??= await startDockerProxy(socketPath);
		return this.#dockerProxy.address;
	}
